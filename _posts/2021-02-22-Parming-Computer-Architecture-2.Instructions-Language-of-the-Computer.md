...
<!--5-1강-->
## Preview
2장에서는 다음과 같은 내용을 배운다.
* 인스트럭션과 인스트럭션 집합 구조
* MIPS ISA의 3가지 인스트럭션 형식과 각 형식에 해당하는 인스트럭션의 종류
  * 산술 연산, 조건문, 반복문, 함수 등을 구현하는 인스트럭션들
* 어떻게 사람이 읽고 쓰는 C 코드가 MIPS 코드를 거쳐 바이너리 코드로 변환되는지
* 어떻게 C 프로그램이 저장되고 실행되는 지
  * 컴파일러, 어셈블러, 링커, 로더, 메모리, 스토리지의 역할
